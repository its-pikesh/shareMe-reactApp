...
import { userCreatedPinsQuery, userQuery, feedQuery, userSavedPinsQuery } from '../utils/data';
  ...
  useEffect(() => {
    if (text === 'Created')
    {
      ...
      client.fetch(createdPinsQuery).then((data) => {
        let filtered = data.filter((item) => item.postedBy._id === userId)
        setPins(filtered)
      })
    }
    else {
      ...
      <div className="relative flex flex-col mb-7">
        <div className="flex flex-col justify-center items-center">
          ...
          <div className="absolute top-0 z-1 right-0 p-2">
            {userId == user._id && (
              <button className="bg-white p-2 rounded-full cursor-pointer outline-none shadow-md" type="button" onClick={() => googleLogout()}>

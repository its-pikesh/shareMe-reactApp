...
const CreatePin = ({ user }) => {
  ...
  const uploadImage = (e) => {
    ...
    if (type === 'image/png' || type === 'image/jpeg' || type === 'image/svg' || type === 'image/JPEG') {
      ...
      <div className="flex flex-1 flex-col gap-6 lg:pl-5 mt-5 w-full">
        ...
        <div className="flex flex-col">
          <div>
            ...
            <select name="" onChange={(e) => { setCategory(e.target.value) }} className="outline-none w-4/5 text-base border-b-2 border-gray-200 rounded-md cursor-pointer">
              <option  value="other" className="bg-white">Select Category</option>
              {categories.map((category,index) => (
                <option key={index} value={category.name} className="text-base border-0 outline-none capitalize bg-white text-black">{ category.name}</option>
              ))}
            </select>
          </div>
